...
int next_command_new(FILE *input, int *op, uint64_t *arg)
{
  int ret;
  char command[64];
...
int new_parser(FILE *input, int *op, 
...
uint64_t range_scan(typename betree<Key, Value>::iterator &betit,
				typename betree<Key, Value>::iterator &beted,
				betree<Key, Value> &b,
		 		FILE *script_output,
...
void benchmark_mixed_file(betree<uint64_t, std::string> &b,
				uint64_t nops, uint64_t number_of_distinct_keys,
			  	uint64_t random_seed,
		  		FILE *script_input, FILE *script_output)
{
  uint64_t overall_timer = 0, timer = 0;

  if (!script_input || !script_output) {
  ...
  for (unsigned int i = 0; i < nops; i++) {
	...
	int r = new_parser(script_input, &op, &t, &ed, new_val);
	if (r == EOF)
		exit(0);
	else if (r < 0)
		exit(4);
	...
	case 4:
		/* range query */
		if (output_required)
			fprintf(script_output, "Range query from %lu to %lu\n", t, ed);
		if (t <= ed) {
			timer_start(timer);
			auto betit = b.lower_bound(t);
			auto beted = b.upper_bound(t);
			timer_stop(timer);
			timer += range_scan(betit, beted, b, script_output, output_required);
			fprintf(script_output, "Done\n");
		} else {
			fprintf(script_output, "DNE\n");
		}
		break;	
	default:
...
int main(int argc, char **argv)
{
  ...
  if (strcmp(mode, "test") == 0)
	...
	benchmark_mixed_file(b, nops, number_of_distinct_keys, random_seed, script_input, script_output);
