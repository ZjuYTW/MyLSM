...
void diskLevel<K, V>::add_runs(vector<diskRun<K,V> *> &runlist, bool last){
    ...
    vector<int> multi_ways(runlist.size());
    ...
    while(h.size != 0){
        ...
        if(lastKey == top.first.key){
            ...
            j++;
            if(j != -1 && last && runs[_cur_run]->map[j].value == V_TOMBSTONE){
                j--;
            }
            runs[_cur_run]->map[j] = top.first;
        }
...
void diskLevel<K, V>::free_merged_runs(vector<diskRun<K,V> *> &toFree){
    ...
    for(int i = _cur_run; i < _run_num; i++){
        diskRun<K, V> *newrun = new diskRun(_run_size, _pageSize, _level, i, _bf_fp);
